...
const sequelize = require('../config/db'); // Import instance của Sequelize

/**
 * Hàm tạo đơn hàng mới
 */
const createOrder = async (req, res) => {
    const { deliveryInfo, rushDeliveryInfo, invoiceDetails, mediaDetails, status } = req.body;
    ...
    try {
        // 1. Tạo DeliveryInfo
        const createdDeliveryInfo = await DeliveryInfo.create(deliveryInfo, { transaction: t });

        // 2. Tạo RushDeliveryInfo nếu có
        if (rushDeliveryInfo) {
            await RushDeliveryInfo.create({
                id: createdDeliveryInfo.id, // Liên kết với DeliveryInfo vừa tạo
                rushInstruction: rushDeliveryInfo.rushInstruction || null,
                rushTime: rushDeliveryInfo.rushTime || null
            }, { transaction: t });
        }

        // 3. Tạo Order
        const createdOrder = await Order.create({ status }, { transaction: t });

        // 4. Tạo Invoice
        const createdInvoice = await Invoice.create({
            mediaTotal: parseFloat(invoiceDetails.mediaTotal),
            vat: parseFloat(invoiceDetails.vat),
            total: parseFloat(invoiceDetails.total),
            orderId: createdOrder.id,
            deliveryInfoId: createdDeliveryInfo.id
            // mediaSubtotal và shippingFee có thể được tính toán thêm nếu cần
        }, { transaction: t });

        // 5. Tạo các mục OrderMedia
        const orderMediaEntries = mediaDetails.map(media => ({
            orderId: createdOrder.id,
            mediaId: media.mediaId,
            quantity: media.quantity
        }));

        await OrderMedia.bulkCreate(orderMediaEntries, { transaction: t });
